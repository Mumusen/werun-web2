 ...
 * @LastEditors  : linxiao
 * @LastEditTime : 2023-03-29 18:37:14
...
<script setup>
// 接收数据
const deptData = defineProps(['deptItem'])
console.log('deptItem', deptData)
// 按钮显示 隐藏
...
const openChildren = row => {
  console.log('打开子级', row)
}
</script>
<template>
  <div class="dept-box" v-if="deptData">
    <div class="dept-box-title h-48px lh-48px">
      LEVEL - {{ deptData.deptItem[0].k + 1 }}
    </div>
    <div class="dept-box-min">
      ...
      >
        ...
        <div class="dept-box-item-r flex justify-center items-center">
          ...
          <template
            v-if="item.editShowFlag && (item.editShow || item.editState)"
          >
      ...
      <!-- <template v-if="deptData.length === 0">
